...
library BiswapLibrary {
    ...
    function getSwapFee(
        ...
        address tokenA,
        address tokenB
    ) internal view returns (uint swapFee) {
    ...
    function getReserves(
        ...
        (uint reserve0, uint reserve1, ) = IBiswapPair(
...
contract BiswapRouter02 is IBiswapRouter02, Ownable {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = BiswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = BiswapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint amountAOptimal = BiswapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
